...
function collect(goodsId)
{
  ...
  $.ajax({
		...
		success: function (result){
					...
					document.getElementById("user_collect").style.bottom = ((r - q) / 2 - p) + "px";
					
					var l=(document.body.clientWidth-220)/2;  
					l+=document.body.scrollLeft;  
					document.getElementById("user_collect").style.left = l + "px";
					
					$("#user_collect_title").html(result.message);
					$("#user_collect").show();
			}
	});
}
...
function addToCart(goodsId, flag) {
  ...
  goods.parent   = (typeof(parentId) == "undefined") ? 0 : parseInt(parentId);
		...
		e.spin($("#spinner2")[0]);
  		if ($("#user_cart_mask")) {
			$("#user_cart_mask").remove()
		}
		var k = ((document.body || document.documentElement).clientHeight + 20) + "px";
		var a = "100%";
		var m = document.createElement("div");
		m.setAttribute("id", "user_cart_mask");
		m.setAttribute("style", "position:absolute;left:0px;top:0px;background-color:rgb(20, 20, 20);filter:alpha(opacity=60);opacity: 0.6;width:" + a + ";height:" + k + ";z-index:9998;"); (document.body || document.documentElement).appendChild(m);
		...
		success: function(res) {
				...
				if (flag){
					location.href = "flow.php";
						...
						var p = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop;
						var r = document.documentElement.clientHeight || document.body.clientHeight;
						document.getElementById("user_cart").style.bottom = ((r - q) / 2 - p) + "px";
						
						var l=(document.body.clientWidth-220)/2;  
						l+=document.body.scrollLeft;  
						document.getElementById("user_cart").style.left = l + "px";
